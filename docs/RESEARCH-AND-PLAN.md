...
**그 무언가가 무엇인지는 아직 측정 없음.** 유력 후보(추정): Studio Play는 클라와 서버가 한
프로세스고, 서버는 매 프레임 200패킷을 디코드하는데 **디코드 경로는 M2의 블록 최적화를 한 번도 안
받았다**. M4 보안 보고서가 디코드를 인코드의 2.5배, 손으로 쓴 리더의 4배로 측정했고,
`Buffer.ensure`는 정확히 그 용도로 쓰였는데 `src/`에 호출자가 없다. 직렬화를 아예 안 하는 `raw`가
30 FPS로 꼴찌인 것도 같은 방향을 가리킨다.

**이 셀에서 원인을 지목하고 고치고 안 움직인 게 두 번째다** (첫 번째는 §3.10-BB의 `allocate`
600회, FF에 취소선). 규칙으로 적어 둘 값이 있다: **마이크로벤치는 부품이 빨라진 걸 증명할 수 있고,
그 부품이 사는 프레임에 대해서는 아무것도 증명하지 못한다.**
