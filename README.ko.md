...
마일스톤 M5가 닫혔습니다. 코덱, transport, 복제가 구현되었고, 외부 보안 감사 두 건의 모든 발견에
처분이 적혔으며(`docs/SECURITY-REPORT*.md`, `docs/milestone/PLAN-M4.md` §9), 그 감사들이 미뤘던 타입
계층을 M5가 닫았습니다 — 생성자의 인자가 타입으로 검사되고, 핸들러의 `ctx`가 `nw.Ctx`이며, 모든 공개
값에 이름 붙일 수 있는 타입이 있고, 최적화 발견 일곱 건이 각자의 프로브와 함께 닫혔습니다
(`bench/residue`). `docs/milestone/PLAN-M5.md` §9에 측정된 전후 표, 의도적으로 열어둔 작업 하나,
측정한 뒤 일부러 하지 않은 다섯 가지가 있습니다. lune 스위트와 Studio 스위트 모두 녹색입니다. 이
저장소는 로컬에서 개발됩니다.
...
`bench/vendor/`에는 벤치마크가 네 라이브러리를 한 세션에서 돌릴 수 있도록 경쟁 라이브러리 셋이
들어 있습니다. Blink, Zap, ByteNet이고 모두 MIT이며 각각 상류 라이선스를 옆에 두었습니다. 어느 것이
생성 출력이고 어느 것이 소스 복사본인지는 [`bench/vendor/README.md`](bench/vendor/README.md)에
있습니다. 배포물에는 아무것도 들어가지 않습니다 — 패키지는 `src`만 빌드합니다.
